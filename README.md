...
> **Status:** Phases 0–2 complete (schema, policy + incident CRUD, idempotent dedup, API-key auth).
> The escalation worker, ack-race handling, dead-lettering, and real channels land in later phases —
> see [Roadmap](#roadmap).
...
| Multiple workers, no double-sends, no message broker | `SELECT … FOR UPDATE SKIP LOCKED` job claiming on `notification_attempt` *(Phase 3)*. |
...
- [ ] **3** Worker loop — `SKIP LOCKED` claiming + step escalation
